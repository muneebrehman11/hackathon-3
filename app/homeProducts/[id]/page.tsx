...
const HomeProducts = async ({ params }: { params: Promise <{ id: string }> }) => {
    ...
    return (
        <div className='flex flex-col lg:flex-row justify-center items-center md:py-4 md:px-6 sm:px-10 gap-28'>
            {/* left content */}
            <div className=''>
                <Image src={urlFor(homeProducts.imageUrl).url()} alt={homeProducts.name} width={506} height={540} className='hover:opacity-90 bg-black ' />
            ...
            <div className='flex-1  space-y-5 '>
                <h3 className='text-xl text-slate-600'>{homeProducts.name}</h3>
                ...
                <p className=' font-bold space-x-2 text-2xl'>
                    <span className='text-slate-400 line-through'>{homeProducts.price_before} </span>
                    <span className='text-green-600 '>{homeProducts.price_after}</span>
                </p>
